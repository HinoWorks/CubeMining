...
namespace PhysicsGameSystem
{
    public class BlockGenerateManager : MonoBehaviour
    {
        ...
        private void GenerateBlock()
        {
            ...
            var blockPrefab = Get_RandomBlockPrefab();
            var newBlock = Instantiate(blockPrefab, InGameManager.Inst.ParentPool) as GameObject;
            var targetBlock = newBlock.GetComponent<MiningTarget_Cube>();
            list_targetBlocks.Add(targetBlock);
            targetBlock.Init(5, 2, 0, 0);
            targetBlock.Set_BlockType(BaseBlockType.Stone, ResourceType.Stone);
